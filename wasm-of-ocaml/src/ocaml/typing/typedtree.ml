...
let rec alpha_pat
  ...
  | Tpat_alias (p1, id, s) ->
   ...
   (* ONES BELOW ARE NEW AND SHOULDN'T BE NEEDED - can possibly use 'as x' to neaten *)
   | Tpat_any -> let pat_desc =
                        shallow_map_pattern_desc { f = fun p -> alpha_pat env p } Tpat_any in
                      {p with pat_desc}
   | Tpat_constant c -> let pat_desc =
                           shallow_map_pattern_desc { f = fun p -> alpha_pat env p } (Tpat_constant c) in
                         {p with pat_desc}
    | Tpat_tuple l -> let pat_desc =
          shallow_map_pattern_desc { f = fun p -> alpha_pat env p } (Tpat_tuple l) in
          {p with pat_desc}
    | Tpat_construct (loc, desc, l) -> let pat_desc =
              shallow_map_pattern_desc { f = fun p -> alpha_pat env p } (Tpat_construct (loc, desc, l)) in
              {p with pat_desc}
    | Tpat_variant (l, pat, row) -> let pat_desc =
              shallow_map_pattern_desc { f = fun p -> alpha_pat env p } (Tpat_variant (l, pat, row)) in
              {p with pat_desc}
    | Tpat_record (l, flag) -> let pat_desc =
              shallow_map_pattern_desc { f = fun p -> alpha_pat env p } (Tpat_record (l, flag)) in
              {p with pat_desc}
    | Tpat_array l -> let pat_desc =
              shallow_map_pattern_desc { f = fun p -> alpha_pat env p } (Tpat_array l) in
              {p with pat_desc}
    | Tpat_lazy pat -> let pat_desc =
              shallow_map_pattern_desc { f = fun p -> alpha_pat env p } (Tpat_lazy pat) in
              {p with pat_desc}
    | Tpat_value v -> let pat_desc =
                  shallow_map_pattern_desc { f = fun p -> alpha_pat env p } (Tpat_value v) in
                  {p with pat_desc}
    | Tpat_exception pat -> let pat_desc =
              shallow_map_pattern_desc { f = fun p -> alpha_pat env p } (Tpat_exception pat) in
              {p with pat_desc}
    | Tpat_or (pat1, pat2, row) -> let pat_desc =
                  shallow_map_pattern_desc { f = fun p -> alpha_pat env p } (Tpat_or (pat1, pat2, row)) in
                  {p with pat_desc}
 (* | d ->
     let pat_desc =
