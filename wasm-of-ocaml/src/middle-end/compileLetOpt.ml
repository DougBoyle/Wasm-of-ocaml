...
let rec apply_mixture_rule matrix =
  ...
  match matrix with
    ...
    | (((p::_, _) as row)::rest) -> if CompileMatchOpt.is_variable_pattern p
      then split_by_test CompileMatchOpt.is_variable_pattern [row] rest
      else if CompileMatchOpt.is_constructor_pattern p
      then split_by_test CompileMatchOpt.is_constructor_pattern [row] rest
...
let rec compile_matrix ~exported fail values matrix =
  ...
  | (v::vs, [(({pat_desc=Tpat_or(_, _, _)} as p)::ps, act)]) ->
    let patterns = CompileMatchOpt.expand_ors p in
