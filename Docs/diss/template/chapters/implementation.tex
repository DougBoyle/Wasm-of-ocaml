...
The mixture rule will group the first two rows to be matched by the constructor rule, but will divide the remaining 3 rows each into separate 1-row matrices. The goal is to jump past one handler to an enclosing one, so \verb|try ... with i ...| blocks are formed in bottom-up order so that matching of the first two rows is enclosed by 3 handlers. \\ % TODO: Add a diagram/pseudocode for this?
