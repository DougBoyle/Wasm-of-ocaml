...
First, the tree is scanned and every \verb|let x = Constant c in ...| or \verb|let x = Ident y in ...| binding where \verb|x| is marked as a local variable is added to a table. These are the ``immediate" values in the linearised IR and represent bindings that do no real work. Every variable use in the tree is looked up in the table, and wherever a corresponding entry is found that variable is replaced with its known value. 
